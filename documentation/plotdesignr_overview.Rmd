...
For this example, three clusters and a mixing parameter of 0.1 is selected (critera for choosing cluster number not shown). Several critera as well as knowledge about the field must be considered in order to create meaningful clusters.
```{r finalize-clusters}
...
One of the most powerful features of `plotdesignr` is the ability to interactively create and simulate various experimental designs. Once the clusters have been choosen, simply click on the map to design new experiments. The goal it to place experiment blocks inside of areas of the same cluster because those areas represent homogenous yield environments.
